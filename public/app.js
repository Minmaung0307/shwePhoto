...
function applyRoleGates() {
  ...
  document
    .querySelectorAll('.gated-pro .btn, .gated-pro input, .gated-pro select')
    .forEach((el) => {
      el.disabled = !isPro;
      el.title = isPro ? '' : 'Pro feature (admin/editor only)';
    });
}
// Apply once at load; will update again on the role-change event
applyRoleGates();
...
function setCanvasSizeByOption() {
  ...
  const h =
    val === '1350' ? 1350 : val === '1920' ? 1080 : val === '3000' ? 2000 : 1080;
  canvas.width = w;
  canvas.height = h;
}
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    switch (currentFilter) {
      ...
      case 'watercolor': {
        ...
        d[i] = gamma(r * 0.9 + g * 0.05 + b * 0.05);
        d[i + 1] = gamma(g * 0.9 + r * 0.05 + b * 0.05);
        d[i + 2] = gamma(b * 0.9 + r * 0.05 + g * 0.05);
        break;
      }
      case 'oil': {
...
function applyFrame() {
  ...
  switch (frameSelect.value) {
    case 'simple':
      canvas.classList.add('frame-simple');
      break;
    case 'ornate':
      canvas.classList.add('frame-ornate');
      break;
    case 'film':
      canvas.classList.add('frame-film');
      break;
  }
}
...
function autoFitAccessories() {
  ...
  const leftEye = avgPoint(pts[36], pts[39]);
...
function placeAccessory(key, pt, angle = 0, pxSize = 40, mirror = false) {
  const el =
    accessories[key] ||
    document.querySelector(`.overlay[data-acc="${key}"]`) ||
    createAccessoryElement(key);
    ...
    (e) => {
      ...
      const onMove = (ev) => {
        const dx = ev.clientX - sx,
          dy = ev.clientY - sy;
        el.style.left =
          startX + dx - stage.getBoundingClientRect().left + 'px';
        el.style.top =
          startY + dy - stage.getBoundingClientRect().top + 'px';
      };
      const onUp = () => {
...
function composeToCanvas() {
  ...
  stage.querySelectorAll('.overlay').forEach((el) => {
    ...
    const fontFamily = el.style.fontFamily || 'system-ui';
    ...
    if (tr && tr !== 'none') {
      ...
      ctx.font = fs * scaleX + 'px ' + (el.classList.contains('textbox') ? fontFamily : 'system-ui');
      ctx.textAlign = 'center';
...
exportBtn.addEventListener('click', async () => {
  if (!baseImg) {
    alert('Please upload an image first to export.');
    return;
  }
  composeToCanvas();
  canvas.toBlob(
...
(function () {
  ...
  sendEmailBtn.addEventListener('click', async () => {
    try {
      if (!window.__lastExportURL) {
        composeToCanvas();
        window.__lastExportURL = canvas.toDataURL('image/png');
      }
      const params = {
        to_email: emailTo.value || 'you@example.com',
        image_url: window.__lastExportURL,
      };
...
async function runAIEdit(kind, extra) {
  if (!baseImg) {
    alert('Please upload an image before running AI.');
    return;
  }
  if (!(ROLE === 'admin' || ROLE === 'editor')) {
    alert('Pro feature — admin/editor only');
    return;
  }
  composeToCanvas();
...
bgReplaceBtn?.addEventListener('click', () =>
  runAIEdit('bg', { prompt: bgPrompt.value || '' })
);
fgReplaceBtn?.addEventListener('click', () =>
  runAIEdit('fg', { prompt: bgPrompt.value || '' })
);
ageRunBtn?.addEventListener('click', () =>
  runAIEdit('age', { mode: ageMode.value || 'older' })
);
celebRunBtn?.addEventListener('click', () =>
  runAIEdit('celeb', { celeb: celebName.value || '' })
);
...
document
  ...
  .forEach((btn) =>
    btn.addEventListener('click', () => {
      currentFilter = btn.dataset.filter; // local state (fixed)
      drawBase();
    })
  );
document
  ...
  .forEach((btn) =>
    btn.addEventListener('click', () => {
      currentTheme = btn.dataset.theme; // local state (fixed)
      drawBase();
    })
  );
...
function setActive(btn) {
  // same group deactivate
  const parent = btn.closest('.btn-col, .topnav');
...
async function handleAction(a, btn) {
  ...
  try {
    switch (group) {
      case 'filter':
        currentFilter = value; // fixed
        drawBase();
        break;

      case 'theme':
        currentTheme = value; // fixed
        drawBase();
        break;

      case 'frame': {
        document.getElementById('frameSelect').value = value;
        const ev1 = new Event('change');
        document.getElementById('frameSelect').dispatchEvent(ev1);
        break;
      }
      ...
      case 'size': {
        ...
        document.getElementById('sizeSelect').dispatchEvent(ev2);
        break;
      }
      ...
      case 'acc':
        if (value === 'clear') {
          ...
          if (accessories?.[value]) {
            accessories[value].remove();
            delete accessories[value];
          } else {
            const el = document.createElement('div');
            el.className = 'overlay';
            el.dataset.acc = value;
            el.textContent = (ACC_DEFS?.[value]?.text) || '★';
            enableDrag(el);
            stage.appendChild(el);
            accessories[value] = el;
            if (typeof autoFitAccessories === 'function') autoFitAccessories();
          }
        }
        break;
      ...
      case 'ai': {
        ...
        busy(true);
        const aiMap = {
          hair_off: { kind: 'fg', extra: { prompt: 'remove hair (clean bald), keep natural head shape, keep background consistent' } },
          hair_short: { kind: 'fg', extra: { prompt: 'short neat haircut, natural look' } },
          hair_long: { kind: 'fg', extra: { prompt: 'long hair, natural flow' } },
          beard_on: { kind: 'fg', extra: { prompt: 'add neat short beard, realistic' } },
          beard_off: { kind: 'fg', extra: { prompt: 'remove beard, clean-shaven' } },
          fg_prompt: { kind: 'fg', extra: { prompt: prompt('Foreground change prompt?', 'formal attire') || 'formal attire' } },
          bg_beach: { kind: 'bg', extra: { prompt: 'sunny beach background, soft bokeh' } },
          bg_studio: { kind: 'bg', extra: { prompt: 'clean studio background, soft shadow' } },
          bg_custom: { kind: 'bg', extra: { prompt: prompt('Background prompt?', 'night city skyline') || 'night city skyline' } },
        };
        const cfg = aiMap[value];
        if (cfg) await runAIEdit(cfg.kind, cfg.extra);
        break;
      }
